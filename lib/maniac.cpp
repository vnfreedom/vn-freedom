...
namespace maniac {
	...
	void play(const std::vector<Action> &actions) {
        ...
        debug("play: entering with %zu actions, game_time=%d ms",
              total_actions, osu->get_game_time());

		while (cur_i < total_actions) {
            // Tolerate a brief transient state (lazer can briefly flicker during map start).
            // Only actually exit if is_playing() returns false 3 times in a row.
            if (!osu->is_playing()) {
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
                if (!osu->is_playing()) {
                    std::this_thread::sleep_for(std::chrono::milliseconds(50));
                    if (!osu->is_playing()) {
                        debug("play: is_playing returned false 3 times, exiting");
                        return;
                    }
                }
            }
