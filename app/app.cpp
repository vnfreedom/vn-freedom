...
static void set_priority_class(int priority) {
    ...
    debug("changed priority class from 0x%lx to 0x%lx", old_priority, GetPriorityClass(proc));
}

static void run(osu::Osu &osu, std::string &message) {
    ...
    maniac::randomize(hit_objects,
        maniac::config.randomization_mean,
        maniac::config.randomization_stddev);
...
int main(int, char **) {
    ...
    auto thread = std::jthread([&message](const std::stop_token &token) {
    ...
    window::start([&message] {
        ImGui::Begin("vn!freedom", nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoResize);

        ImGui::Text("vn!freedom");
        ImGui::SameLine();
        ImGui::TextDisabled("osu! stable");
        horizontal_break();
        ...
        ImGui::Checkbox("Stealth Mode (Hide from Task Manager)", &maniac::config.stealth_mode);
        ImGui::SameLine();
        help_marker("When enabled, automatically hides the app window from Taskbar and Task Manager Apps whenever Task Manager starts. Disguises process as Runtime Broker. Press [INSERT] anytime to toggle show/hide.");
        ...
        ImGui::TextDisabled("vn!freedom | Hotkey: [INSERT] show/hide");
