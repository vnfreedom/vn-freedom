...
void window::start(const std::function<void()> &body) {
    ...
    WNDCLASSEX wc = {sizeof(WNDCLASSEX), CS_CLASSDC, WndProc, 0L, 0L, GetModuleHandle(NULL), NULL,
            NULL, NULL, NULL, _T("vn!freedom"), NULL};
    wc.hIcon = ::LoadIcon(wc.hInstance, MAKEINTRESOURCE(IDI_APP_ICON));
    wc.hIconSm = ::LoadIcon(wc.hInstance, MAKEINTRESOURCE(IDI_APP_ICON));
    ::RegisterClassEx(&wc);
    HWND hwnd = ::CreateWindow(wc.lpszClassName, _T("vn!freedom"), WS_OVERLAPPEDWINDOW, 100, 100, 550,
            450, NULL, NULL, wc.hInstance, NULL);
